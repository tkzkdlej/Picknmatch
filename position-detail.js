...
(function () {
  ...
  var POSITION_DATA = {
    chemical: {
      ...
      mainTasks: [
        "원유·가스 정제와 기초·고부가 화학제품(플라스틱·정밀화학 등) 생산",
        "친환경·바이오 기반 소재로의 전환 가속",
      ],
      companies: [
    ...
    heavyIndustry: {
      ...
      mainTasks: [
        "대형 선박 건조 및 해양·산업 플랜트 건설",
        "초대형 기계·장비 제조와 설치",
        "자율항해·수소선 등 차세대 조선·기계 핵심 기술",
      ],
      companies: [
    ...
    semiconductor: {
      ...
      mainTasks: [
        "데이터 연산·저장용 반도체 칩 설계(팹리스) 및 파운드리 제조",
        "AI 시대 수요에 대응하는 HBM 등 첨단 메모리·연산 반도체",
      ],
      companies: [
    ...
    secondaryBattery: {
      ...
      mainTasks: [
        "전기차·ESS용 셀·모듈·팩 설계·양산 및 품질 관리",
        "양극·음극재·분리막 등 핵심 소재·장비 밸류체인",
      ],
      companies: [
    ...
    ecoFriendly: {
      ...
      mainTasks: [
        "탄소 저감을 위한 순환경제·자원 순환 체계 구축",
        "폐기물 처리 솔루션과 친환경 에너지 전환 서비스",
      ],
      companies: [
    ...
    materials: {
      ...
      mainTasks: [
        "배터리 양·음극재, 디스플레이 부품, 반도체 웨이퍼 등 첨단 산업 핵심 소재",
        "해당 소재의 연구·개발·제조 일원화",
      ],
      companies: [
    ...
    defense: {
      ...
      mainTasks: [
        "자주포·전투기·미사일 등 첨단 무기체계 개발·생산",
        "AI 무인체계·위성통신 등 신규 방산 영역 확대",
      ],
      companies: [
    ...
    distribution: {
      ...
      mainTasks: [
        "온·오프라인 커머스 플랫폼 운영",
        "물류·배송 네트워크 최적화, AI 기반 개인화 쇼핑 경험",
      ],
      companies: [
  ...
  function populatePanel(panel, key) {
    ...
    var tasksEl = panel.querySelector("[data-inline-tasks]");
    if (tasksEl) {
      var taskLines = data.mainTasks || [];
      if (taskLines.length === 0) {
        ...
        tasksEl.innerHTML =
          '<h3 class="position-inline-detail__subhead">주요 업무</h3><ul class="position-inline-detail__task-list">' +
          taskLines
            .map(function (line) {
